...
class Blender(object):
        ...
        def base_learners(self,Xtrain, Xtest, y, xgb_parameters1, base_epochs, batch,
        keras_base_models):
            ...
            ################################################################################
                ...
                ################################################################################
                    ...
                    for j, (train_idx, valid_idx) in enumerate(kf):
                        ...
                        gbm = xgb.train(xgb_parameters1[b],dtrain,
        ...
        keras_secondlevel_models):
            ...
            #######################################################################################################################
                    ...
                    for j, (train_idx, valid_idx) in enumerate(kf2):
                        ...
                        dvalid = xgb.DMatrix(X_valid, y_valid)
                        watchlist = [(dtrain, 'train'), (dvalid, 'eval')]
                        gbm = xgb.train(xgb_parameters2[d], dtrain, evals=watchlist, verbose_eval=True)
                        y_pred = gbm.predict(dvalid)
                        score = log_loss(y_valid.tolist(), y_pred)
                        print("{:.5f}".format(score), end=' ')
